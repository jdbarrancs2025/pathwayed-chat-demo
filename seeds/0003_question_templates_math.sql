...
values
  ('linear-equation-solve',
   ...
   '{"kind":"template_math","schemaVersion":1,"responseType":"multiple_choice","stemTemplate":"If $\\,{a}x{±b} = {c}$, what is the value of $x$?","slots":[{"name":"a","min":2,"max":9},{"name":"x","min":-9,"max":9,"exclude":[0]},{"name":"b","min":-9,"max":9,"exclude":[0]}],"derived":[{"name":"c","formula":"a*x + b"},{"name":"cMinusB","formula":"c - b"}],"answerFormula":"x","answerFormat":"integer","constraints":["a != 0"],"solutionTemplate":"Start with $\\,{a}x{±b} = {c}$. Subtract the constant from both sides: $\\,{a}x = {cMinusB}$. Divide by ${a}$: $x = {answer}$."}'::jsonb,
   '[{"formula":"c - b","misconception_token":"forgot-to-divide"},{"formula":"-x","misconception_token":"sign-error"},{"formula":"x + b","misconception_token":"mishandled-constant"}]'::jsonb,
   'published', 1)
...
values
  ('percent-of',
   ...
   '{"kind":"template_math","schemaVersion":1,"responseType":"multiple_choice","stemTemplate":"What is ${p}\\%$ of ${n}$?","slots":[{"name":"p","min":5,"max":90,"step":5},{"name":"n","min":20,"max":200,"step":20}],"answerFormula":"p * n / 100","answerFormat":"integer","solutionTemplate":"${p}\\%$ of ${n}$ is $\\frac{{p}}{100} \\times {n} = {answer}$."}'::jsonb,
   '[{"formula":"p * n / 10","misconception_token":"decimal-place-error"},{"formula":"n - p * n / 100","misconception_token":"complement-instead-of-percent"},{"formula":"p * n / 50","misconception_token":"halved-the-base"},{"formula":"p","misconception_token":"answered-the-percent"}]'::jsonb,
   'published', 1)
on conflict (code) do update set
  skill_id        = excluded.skill_id,
  sat_alignment   = excluded.sat_alignment,
