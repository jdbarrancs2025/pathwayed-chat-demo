...
import {
  ...
  buildPrepQuestion,
  seedForSlot,
} from './analogyTemplates'

const DIFFICULTIES = new Set(['easy', 'medium', 'hard'])

describe('prep analogy templates', () => {
  for (const skill of PREP_ANALOGY_SKILLS) {
      ...
      it('every item: 4 distinct choices, exactly one correct, 3 distinct mapped tokens, valid difficulty', () => {
        ...
        for (const item of skill.items) {
          ...
          for (const tok of tokens) {
            // Self-contained: assert the token maps to a local explanation.
            // (misconceptions.ts merges ANALOGY_MISCONCEPTIONS centrally afterward,
            // so explainMisconception() is verified there, not here.)
            expect(ANALOGY_MISCONCEPTIONS[tok as string]).toBeTruthy()
          }
