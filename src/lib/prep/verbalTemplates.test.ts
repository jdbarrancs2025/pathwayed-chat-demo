...
import { PREP_VERBAL_SKILLS, buildPrepQuestion, seedForSlot } from './verbalTemplates'
import { explainMisconception } from '../misconceptions'

const DIFFICULTIES = new Set(['easy', 'medium', 'hard'])

describe('prep verbal templates', () => {
      ...
      it('every item: 4 distinct choices, exactly one correct, 3 distinct mapped tokens, valid difficulty', () => {
        ...
        for (const item of skill.items) {
          ...
          for (const tok of tokens) {
            expect(explainMisconception(tok)).toBeTruthy() // every token has an explanation
          }
