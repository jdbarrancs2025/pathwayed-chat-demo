...
describe('prep logic + verbal classification templates', () => {
  for (const skill of PREP_LOGIC_SKILLS) {
    describe(`${skill.slug} (${skill.items.length} items)`, () => {
      ...
      it('covers all three difficulty tiers', () => {
        const tiers = new Set(skill.items.map((i) => i.difficulty))
  ...
  it('LOGIC_MISCONCEPTIONS explains every token used by both skills', () => {
    const used = new Set<string>()
    for (const skill of PREP_LOGIC_SKILLS) {
      for (const item of skill.items) {
        for (const c of item.choices) {
          if (c.misconception_token) used.add(c.misconception_token)
        }
      }
    }
    for (const tok of used) {
      expect(LOGIC_MISCONCEPTIONS[tok]).toBeTruthy()
    }
  })
})
