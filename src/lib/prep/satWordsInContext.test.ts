...
import {
  ...
  buildPrepQuestion,
  seedForSlot,
} from './satWordsInContext'

const DIFFICULTIES = new Set(['easy', 'medium', 'hard'])

describe('prep sat words in context templates', () => {
  for (const skill of PREP_SAT_WORDS_IN_CONTEXT_SKILLS) {
      ...
      it('every item: 4 distinct choices, exactly one correct, 3 distinct mapped tokens, valid difficulty', () => {
        ...
        for (const item of skill.items) {
          ...
          for (const tok of tokens) {
            ...
            expect(SAT_WORDS_IN_CONTEXT_MISCONCEPTIONS[tok as string]).toBeTruthy()
          }
