/**
 * Authoring script (run with Bun): emits the GRADE 5 & 6 math question seed from
 * the curated pool in src/lib/grade5_6MathTemplates.ts. NOT run at request time.
 *
 *   bun run scripts/build-grade5-6-math-seeds.ts
 *
 * Writes seeds/0014_grade5_6_math_questions.sql. Tops up grades 5 and 6 toward the
 * grade-3 shape (~8 skills each): grade-5 decimals/fractions/coordinate plane/
 * expressions, grade-6 integers/dividing fractions/one-step equations/geometry.
 * Reuses the CCSS math domains from seeds 0009 (3-5) and 0011 (6-8); no duplicate
 * domain rows. Idempotent; skills upsert ON CONFLICT (slug).
 * Apply with: bun run db:seed seeds/0014_grade5_6_math_questions.sql
 */
import { createHash } from 'node:crypto'
import { writeFileSync } from 'node:fs'
import { dirname, join } from 'node:path'
import { fileURLToPath } from 'node:url'
import { GRADE_5_6_MATH_SKILLS } from '../src/lib/grade5_6MathTemplates'
...
// Reuse domains from seeds 0009 (3-5) and 0011 (6-8).
const DOMAIN: Record<string, { name: string; band: string }> = {
  ...
  'dom-math-geo': { name: 'Geometry', band: '3-5' },
...
const DOMAIN_BY_SLUG: Record<string, string> = {
  'decimal-place-value': 'dom-math-nbt',
  'decimal-operations': 'dom-math-nbt',
  'multiply-divide-whole-5': 'dom-math-nbt',
  'add-subtract-fractions-unlike': 'dom-math-frac',
  'multiply-divide-fractions': 'dom-math-frac',
  'coordinate-plane': 'dom-math-geo',
  'numerical-expressions': 'dom-math-oat',
  'integers-rational-numbers': 'dom-math-ns7',
  'divide-fractions': 'dom-math-ns7',
  'one-step-equations': 'dom-math-ee7',
  'area-surface-volume-6': 'dom-math-geo7',
}
const md5uuid = (key: string) => `md5('pe-aos:${key.replace(/'/g, "''")}')::uuid`

const HEADER = `-- PathwayEd — Question Engine: GRADE 5 & 6 math skills + questions (GENERATED).
--
...
for (const skill of GRADE_5_6_MATH_SKILLS) {
...
writeFileSync(join(seedsDir, '0014_grade5_6_math_questions.sql'), sql)
console.log(`Wrote seeds/0014 (${count} questions across ${GRADE_5_6_MATH_SKILLS.length} skill(s)).`)
