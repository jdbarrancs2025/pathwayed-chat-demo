...
-- row in public.students; every table keys on student_id -> public.students(id)
-- and authorizes exactly like the other per-child tables, through
-- public.owns_student() (keys on parent_id = auth.uid()).
--
-- What this migration establishes:
--   prep_entitlements    — which module a child may use, and its billing status.
--                          Written ONLY by the Stripe webhook as the service role
--                          (bypasses RLS); authenticated can read it but not write
--                          it, so a parent cannot self-grant a paid module.
...
--
-- RLS + grants: every table gets owns_student() policies. prep_entitlements is
-- read-only to authenticated (only the service-role webhook writes it); the other
-- three get explicit select/insert/update/delete grants. Grants are stated
...
-- subscription item as the service role; the client only reads its own
-- children's entitlements under RLS to know which modules are unlocked.
...
-- Read-only for authenticated: entitlements are server-owned. There is
-- deliberately no insert/update/delete policy here — the webhook writes this
-- table as the service role, which bypasses RLS and grants.
...
-- Any insert/update/delete policies from an earlier draft are removed, so the
-- read-only grant below is the whole story for authenticated.
...
grant select on public.prep_entitlements to authenticated;
revoke insert, update, delete on public.prep_entitlements from authenticated;

-- 2) prep_settings -----------------------------------------------------------
-- Per-child, per-module settings. Composite primary key (student_id, module_id)
-- gives one settings row per child per module and indexes student_id lookups.
...
create index if not exists prep_attempts_student_id_idx
  on public.prep_attempts (student_id);
create index if not exists prep_attempts_student_module_idx
  on public.prep_attempts (student_id, module_id);
...
create unique index if not exists prep_attempts_one_in_progress_idx
  on public.prep_attempts (student_id, module_id, section_id)
...
-- feedback. Indexed on student_id for a child's essay history.
...
create index if not exists prep_essay_attempts_student_id_idx
